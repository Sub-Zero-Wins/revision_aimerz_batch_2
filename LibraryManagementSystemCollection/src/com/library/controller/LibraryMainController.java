...
public class LibraryMainController {
	...
	static {
		UserDAO.loadAdmin(new User(1,"admin", "admin@lib.in", "admin@123","ADMIN"));
	...
	private static void viewAccounts() {
		List<Account> accounts = accountService.getUserAccounts(loggedInUser.getId());
		if (accounts == null) {
		...
		for (Account account : accounts) {
			System.out.println("Account ID: " + account.getTxnId() + " | Book ID: " + account.getBookId() + 
					" | Quantity: " + account.getDueDate() + " | Total Price: $" + account.getFine()+ " | IssueStatus: $" + account.getStatus()) ;
		}
	}
